...
class ecodevice extends eqLogic {
	...
	public function getPhase($gceid, $mock_test = 0)
	{
		// Add ability to mock and tests the process without Jeedom
		// $mock_test=0: Normal process
		// $mock_test=1: Run automated tests with mocked files Mono Phase
		// $mock_test=2: Run automated tests with mocked files Tri Phase
		if ( $this->getIsEnable() )
		{
			log::add('ecodevice','debug','get '.$this->getUrl($mock_test). 'protect/settings/teleinfo'.$gceid.'.xml');
			switch ($mock_test) {
		    	case 0:
					$this->xmlStatus = @simplexml_load_file($this->getUrl($mock_test). 'protect/settings/teleinfo'.$gceid.'.xml');
					break;
				case 1:
					$this->xmlStatus = @simplexml_load_file('./test/ecodevice/teleinfo_mono.xml');
					break;
				case 2:
                    $this->xmlStatus = @simplexml_load_file('./test/ecodevice/teleinfo_tri.xml');
					break;
		   	}
			if ( $this->xmlStatus === false )
				throw new Exception(__('L\'ecodevice ne repond pas.',__FILE__)." ".$this->getName());
	...
	public static function daemon() {
		...
		if ( $endtime - $starttime < config::byKey('temporisationLecture', 'ecodevice', 60, true) )
		{
			usleep(floor((config::byKey('temporisationLecture', 'ecodevice') + $starttime - $endtime)*1000000));
		}
	}
